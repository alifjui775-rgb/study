...
export default defineConfig(({ mode }) => {
  // Load env variables into process.env so they are available to the API handler
  ...
  return {
    ...
    plugins: lazyPlugins(async () => [
      ...
      VitePWA({
        ...
        includeAssets: ["favicon.ico", "apple-touch-icon-180x180.png", "pwaicon.png", "icon/*.png"],
        manifest: {
          name: "MNR Study",
          short_name: "MNR Study",
          description: "স্বপ্ন পূরণের পথে, সকল কিছু একসাথে",
          theme_color: "#0f0f0f",
          start_url: "/",
          scope: "/",
          display: "standalone",
          ...
          shortcuts: [
            {
              name: "ক্যালেন্ডার",
              short_name: "ক্যালেন্ডার",
              description: "এডমিশনের সকল তথ্য একত্রে",
              url: "/calendar",
              icons: [{ src: "/icon/calendar.png", sizes: "192x192 512x512", type: "image/png" }],
            },
            {
              name: "প্রশ্নব্যাংক",
              short_name: "প্রশ্নব্যাংক",
              description: "বিগত বছরের একাডেমিক ও এডমিশনের প্রশ্ন সমাধান",
              url: "/qb",
              icons: [{ src: "/icon/qb.png", sizes: "192x192 512x512", type: "image/png" }],
            },
            {
              name: "ড্যাশবোর্ড",
              short_name: "ড্যাশবোর্ড",
              description: "আপনার পড়াশোনার অগ্রগতি, কোর্সসমূহ ও পরীক্ষার পারফরম্যান্স ওভারভিউ",
              url: "/dashboard",
              icons: [{ src: "/icon/dashboard.png", sizes: "192x192 512x512", type: "image/png" }],
            },
            {
              name: "তথ্যভান্ডার",
              short_name: "তথ্যভান্ডার",
              description: "এডমিশনের সকল পাবলিক, প্রাইভেট ও কলেজের তথ্য একত্রে",
              url: "/public",
              icons: [
                { src: "/icon/information.png", sizes: "192x192 512x512", type: "image/png" },
              ],
            },
          ],
        },
        ...
        configureServer(server: any) {
          server.middlewares.use((req: any, res: any, next: any) => {
            ...
            const isApiRoute =
              ...
              pathname === "/api/practice-exam/start" ||
              pathname === "/api/practice-exam/submit";
            ...
            if (isApiRoute && req.method === "POST") {
              ...
              req.on("end", async () => {
                ...
                try {
                  ...
                  console.error("Failed to parse request body in Vite API mock:", e);
                }
                ...
                try {
                  if (pathname === "/api/auth/callback") {
                    await authCallbackHandler(vercelReq as any, vercelRes as any);
                  } else {
                    next();
                    return;
                  }
                } catch (err: any) {
                  console.error("Error in Vite API Mock:", err);
                  res.statusCode = 500;
                  res.setHeader("Content-Type", "application/json");
    ...
    build: {
      ...
      rollupOptions: {
        output: {
          // Rolldown code splitting (replaces the legacy manualChunks shim):
          // groups are evaluated in order, so the react-core group must stay
          // FIRST — otherwise Rolldown absorbs the React CJS runtime and the
          // JSX runtime into the first vendor chunk that requires them
          // (charts-vendor / editor-vendor), forcing ~205KB of unrelated
          // vendor code onto every page. includeDependenciesRecursively:false
          // keeps each group to exactly its tested packages, so shared small
          // deps (e.g. clsx) are chunked by the default algorithm instead of
          // being merged into a forced vendor chunk.
          codeSplitting: {
