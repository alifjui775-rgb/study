-- ============================================================
-- Migration: Strict RLS Write Isolation & Server-Side Creation
-- ============================================================

-- 1) Strict RLS Policies on student_practice_exams (SELECT ONLY for client)
...
DROP POLICY IF EXISTS student_practice_exams_delete_own ON public.student_practice_exams;
...
-- 2) Strict RLS Policies on student_exam_answers (SELECT ONLY for client)
ALTER TABLE public.student_exam_answers ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS sea_practice_insert_own ON public.student_exam_answers;
DROP POLICY IF EXISTS sea_practice_update_own ON public.student_exam_answers;
DROP POLICY IF EXISTS sea_practice_delete_own ON public.student_exam_answers;

DROP POLICY IF EXISTS sea_practice_select_own ON public.student_exam_answers;
CREATE POLICY sea_practice_select_own
  ON public.student_exam_answers
  FOR SELECT
  USING (
    student_id::text = (
      current_setting('request.headers', true)::json ->> 'x-user-id'
...
-- 3) Updated start_practice_exam RPC (SECURITY DEFINER creates student_practice_exams & practice_exam_sessions)
...
BEGIN
  ...
  s_list AS (
    SELECT sname, row_number() OVER () AS rn, count(*) OVER () AS total_cnt
    FROM subject_names
  )
  SELECT coalesce(
    string_agg(
